...
namespace tl
{
    namespace library
    {
        ...
        LibraryHandle loadLibrary(const std::string& libraryPath)
        {
            ...
            LibraryHandle libraryHandle = dlopen(libraryPath.c_str(), RTLD_LAZY);
            if (libraryHandle == nullptr)
            {
                throw std::runtime_error("Failed to load library: " + libraryPath + ", error: " + (dlerror() ? dlerror() : "unknown"));
            }
            return libraryHandle;
#else
        ...
        void* findFunction(LibraryHandle libraryHandle,
                           const std::string& functionName)
        {
#ifdef _WIN32
            FARPROC functionPointer = GetProcAddress(libraryHandle,
                                                     functionName.c_str());
            ...
            if (functionPointer == nullptr)
            {
                throw std::runtime_error("Failed to find function: " + functionName + ", error: " + (dlerror() ? dlerror() : "unknown"));
            }
