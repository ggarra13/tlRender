...
namespace tl
{
    namespace ndi
    {
        ...
        void System::_init(const std::shared_ptr<system::Context>& context)
        {
            ...
            pNDILib = static_cast<NDIlib_v5*>(library::findFunction(p.handle,
                                                                    "NDIlib_v5_load"));
            if (!pNDILib)
            {
            ...
            p.thread = std::thread(
                [this]
                {
                    ...
                    while (p.running)
                    {
                        ...
                        p_sources = NDIlib_find_get_current_sources(p.NDIfind, &no_sources);

                        for (unsigned i = 0; i < no_sources; ++i)
                        {
                            const std::string deviceName = p_sources[i].p_ndi_name;
                            ...
                            deviceInfo.pixelTypes.push_back(device::PixelType::_8BitBGRA);
                            deviceInfo.pixelTypes.push_back(device::PixelType::_8BitYUV);
